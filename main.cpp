...
//DescriptorHeap作成
ID3D12DescriptorHeap* CreateDescriptorHeap(ID3D12Device* dexice, D3D12_DESCRIPTOR_HEAP_TYPE heapType, UINT numDescriptors, bool shaderVisidle);
...
//DescriptorHandleの取得の関数
D3D12_CPU_DESCRIPTOR_HANDLE GetCPUDescriptorHandel(ID3D12DescriptorHeap* descriptorHeap, uint32_t descriptorSize, uint32_t index);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//HRESULTはWindows系のエラーコードであり、関数が成功したかどうかをSUCCEEDEマクロで判定出来る
	...
	//RootSignature作成
	D3D12_ROOT_SIGNATURE_DESC descriptonRootSignature{};
	descriptonRootSignature.Flags =
	...
	descriptonRootSignature.pStaticSamplers = staticSamplers;
	descriptonRootSignature.NumStaticSamplers = _countof(staticSamplers);
	...
	descriptonRootSignature.pParameters = rootParameters; //ルートパラメータへのポインタ
	descriptonRootSignature.NumParameters = _countof(rootParameters); //配列の長さ
	...
	*materialData = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
	...
	hr = D3D12SerializeRootSignature(&descriptonRootSignature, D3D_ROOT_SIGNATURE_VERSION_1, &signatureBlob, &errorBlob);
	if (FAILED(hr)) {
	...
	//Shaderをコンパイルする
	IDxcBlob* vertexShaderBlob = CompileShader(L"Particle.VS.hlsl", L"vs_6_0", dxcUtils, dxcCompiler, includeHandle);
	assert(vertexShaderBlob != nullptr);

	IDxcBlob* pixelShaderBlob = CompileShader(L"Particle.PS.hlsl", L"ps_6_0", dxcUtils, dxcCompiler, includeHandle);
...
#pragma region 球体
	...
	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex) {
		...
		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex) {
			...
			// b 3
			vertexData[startIndex + 3] = vertexData[startIndex + 1];
			
			// d 4
			vertexData[startIndex + 4].position.x = std::cos(lat + kLatEvery) * std::cos(lon + kLonEvery);
			vertexData[startIndex + 4].position.y = std::sin(lat + kLatEvery);
			vertexData[startIndex + 4].position.z = std::cos(lat + kLatEvery) * std::sin(lon + kLonEvery);
			vertexData[startIndex + 4].position.w = 1.0f;
			vertexData[startIndex + 4].texcoord = { float(lonIndex + 1) / float(kSubdivision),1.0f - float(latIndex + 1) / float(kSubdivision) };

			// c 5
			vertexData[startIndex + 5] = vertexData[startIndex + 2];
		}
	}
...
#pragma endregion
	...
	D3D12_CPU_DESCRIPTOR_HANDLE instancingSrvHandleCPU = GetCPUDescriptorHandel(srvDescriptorHeap, desriptorSizeSRV, 3);
...
#pragma region 球体
			...
			// 描画に使う頂点データを設定してる、〇〇viewはＧＰＵが何のデータなのかが分からないのでそれを教えるためのもの
			...
			////描画！(Draw/ドローコール)。　3頂点で1つのインスタンス。インスタンスについて今後
